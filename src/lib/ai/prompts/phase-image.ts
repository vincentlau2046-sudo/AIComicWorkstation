export function buildPhaseR2IPrompt(params: {
  ...
  const { characterName, phaseName, visualChanges, templateDescription } = params;
  ...
  if (isChinese) {
    ...
    return `${characterName}（${phaseName}）。${changesLine}。保持相同的面部骨骼结构、眼型、鼻型、唇形和身体比例；保持相同的肤色和体格；保持相同的画风和光照质量；保持纯白背景和专业摄影棚布光。其他一切保持不变。`;
  }
  ...
  return `${characterName} (${phaseName}). ${changesLine}. ${preserveLine}. Keep everything else unchanged.`;
}
