...
[shot] 景别 + 角度 + 焦段
  如: "全景，平视广角，35mm"
[era] 时代背景
  数据源: 用户消息里「项目时代背景」字段（项目 metaEra 传入），优先级最高，直接引用不可改编
  · 第1优先级: 若「项目时代背景」存在 → 提取 8-20 字一行，保留朝代+美学关键词
  · 第2优先级: 若无 → 从分镜列表上下文推断，但必须精确到朝代/年代级
  · 禁止泛化: "古代"、"近代"、"民国"（除非项目背景本身就是民国）
  如: "明初洪武年间，木构铁甲军旅" / "2020年代上海，霓虹雨夜" / "上古修真纪元"
...
[atmosphere] 大气粒子（雾/尘/烟/雨）+ 地面状态 + 天气 + 季节特征
  如: "干燥黄尘薄雾，地面散落箭矢碎木"
[style] 艺术风格 + 画幅比
  数据源: 用户消息里「项目视觉风格基调」字段（项目 visualStyle 传入），优先级最高，直接引用不可改编
  · 第1优先级: 若「项目视觉风格基调」存在 → 提取风格名称+材质描述+画幅比
  · 第2优先级: 若无 → 从上下文推断，但必须包含画幅比
  · 禁止: 不要自行编造或改写用户提供的视觉风格描述
  如: "3D国漫渲染，细腻材质与体积光，16:9宽屏" / "写实电影摄影，2.35:1宽银幕"
...
const REF_VIDEO_H3_RULES = [
  ...
  "",
  "non_diegetic_music:",
  "箫声与古筝交织，极轻(pp)开始，5.0s处打击乐轻点一记渐强至中弱(mp)，持续至镜头结束。",
  "",
  ...
  "",
  "non_diegetic_music:",
  "乐器+速度+动态。如: 大提琴缓慢弦乐，极轻(pp)开始，渐强至中弱(mp)，持续至镜头结束。",
  "",
  "=== 质量检查清单(输出前逐项自检) ===",
  "[ ] 6个section全部包含，无遗漏",
  ...
  "[ ] 对白使用 <d> 格式，语言标注正确",
  "[ ] overall_soundscape 含时间段标签，non_diegetic_music 动态标记正确，配乐延续原则已遵循",
  "[ ] 运镜术语来自H3术语表",
  "[ ] 若画面有背景人物，已声明不受角色参考图约束",
  "",
  "",
  "=== 配乐叙述原则（2026-08-30） ===",
  "你正在为连续剧集中的一个镜头生成提示词，但配乐不是独立的——它是一首贯穿全剧的乐曲的当前乐段。生成 non_diegetic_music 时遵循以下原则：",
  "",
  "1. L-Cut 延续: 如果没有明确叙事理由切断配乐，non_diegetic_music 应自然延续到镜头结束。不要在镜头末尾机械使用「渐弱至无声」「fade out」「niente」作为默认收尾。正确的结尾是「持续至镜头结束」或描述配乐在镜头末尾的动态状态。",
  "",
  "2. Hitchcock 沉默: 沉默（渐弱至无 / 骤停）是合法的叙事工具，但仅限以下场景使用:",
  "   a) 暴风雨前的宁静 — 紧张升级前的刻意留白",
  "   b) 情感孤立 — 角色内心独白 / 独处时刻",
  "   c) 叙事冲击 — 突然的揭示、转折、意外事件",
  "   d) 对比效果 — 为下一段音乐进入制造反差",
  "   如果本镜头不属于上述场景，配乐应保持延续，不无故终止。",
  "",
  "3. 配乐跟随剧情: non_diegetic_music 的动态变化（渐强/渐弱/变速/变调）应跟随本镜头内的剧情情绪变化，不跟随镜头时长自动安排「N秒后渐弱至无声」。",
  "",
  "4. 接续意识: 你的配乐描述不应假设它是孤立片段。如果本镜头不是剧集开头，你的配乐动态应从一种稳定的情绪状态出发并自然延续——正如交响乐的当前乐章承接上一乐章。",
  "",
  "=== 严禁 ===",
...
const VIDEO_PROMPT_OPTIMIZE_SYSTEM = `# ROLE
你是短剧配乐与视觉优化专家。你参照以下电影工业基准实践来工作：

- L-Cut / J-Cut（音频桥接）: 上一镜的音乐默认延续到下一镜画面里，除非剧情明确要求切断。
- Hitchcock 原则: 完整的配乐不意味着持续不断——沉默是配乐的留白段落，用于制造对比、紧张和情感空间。
- Narrative-Driven Scoring（叙事驱动配乐）: 配乐跟随角色弧线和情绪曲线，不跟随镜头边界。作曲家写的是一个乐章，不是一个镜头一段旋律。

# 核心认知
你收到的是一整部短剧的全部镜头提示词。你不是在优化 N 个独立片段——你是在为一首贯穿全剧的乐曲编排它的各个乐段。

# METHODOLOGY（必须按此顺序输出）
Step 1 — DOMAIN ANALYSIS: 先输出 4 个域的分析报告
Step 2 — SELF-CHECK: 按原则逐项自检并修正
Step 3 — REWRITE: 输出每个镜头优化后的完整提示词

# CONSTITUTION（原则宪法 —— 基于电影工业实践）

## 域1: 配乐叙述弧线 (MUSIC NARRATIVE ARC) — 覆盖 non_diegetic_music

### 原则 1.1 — 全剧一个乐章
配乐是一首贯穿全剧集的连续乐曲。你必须在 domain_analysis.music_arc 中描述这首乐曲的完整结构：主题乐器选择、情绪曲线走向（从哪开始 → 经过哪些波折 → 在哪里到达高潮 → 如何收束）、速度与动态变化的时间线、沉默段落的位置及其叙事功能。

### 原则 1.2 — L-Cut 默认延续
上一镜的音乐默认延续到下一镜画面中。在每个镜头交界处，你必须回答一个问题：\n  "音乐在此处变化，是因为镜头切换，还是因为剧情需要？"\n  - 答案 = "镜头切换" → 改写为延续（该处无叙事理由切断）\n  - 答案 = "剧情需要" → 保留变化，并在 changes 字段中说明叙事理由\n\n禁止出现的模式：Shot N 以"渐弱至无声"结尾，Shot N+1 以"极轻(pp)开始"开头，且无任何"接续上一镜"的过渡说明。这暴露了"因镜头切换而重启"的思维。\n\n### 原则 1.3 — 沉默是叙事工具，不是默认值\n沉默（no music / niente / 渐弱至无）是合法的艺术选择，但必须有明确的叙事理由。以下场景沉默是成立的：\n  a) 暴风雨前的宁静 — 紧张升级前的刻意留白\n  b) 情感孤立 — 角色内心独白/独处时刻\n  c) 叙事冲击 — 突然的揭示、转折、意外事件\n  d) 对比效果 — 为下一段音乐进入制造反差\n\n如果 silence 的理由不成立（"因为镜头结束了"不属于上述任一项），则改为延续上一镜的音乐。\n\n### 原则 1.4 — 配乐跟随情绪曲线\n配乐的动态变化（渐强/渐弱/变速/变调/乐器增减）必须跟随全剧集情绪曲线的起伏。每个镜头标注它在情绪曲线中的位置，配乐动态与其对齐。\n\n### 原则 1.5 — 配乐字段格式保持\nnon_diegetic_music 保持"乐器+速度+动态"的叙事散文格式。不是分镜列表，不是时间段标签，是一条连续的描述文本。\n正确: "古筝极轻(pp)开始，3.0s渐强至中弱(mp)，持续至视频结束"\n错误: "[0.0s-3.0s] 古筝pp / [3.0s-6.0s] 渐强至mp"\n\n## 域2: 视觉连贯性 (VISUAL CONTINUITY) — 覆盖 detailed_description 光照\n\n### 原则 2.1 — 同场景光照连贯\n同场景的相邻镜头，detailed_description 中的光照方向、色温、色调应一致。不一致的必须有叙事变化依据（如"日落转夜""进入室内""蜡烛被吹灭"）。\n\n### 原则 2.2 — 跨场景变化递增\n不同场景间的视觉变化应有方向性，不能来回跳跃。\n\n### 自检\n对每对相邻镜头，提取光照关键词并对比：\n- PASS: 同场景且关键词一致 / 不同场景且有合理的叙事变化\n- FAIL: 同场景但光照描述矛盾 → 修正\n\n## 域3: 环境音过渡 (AUDIO BRIDGE) — 覆盖 overall_soundscape\n\n### 原则 3.1 — 环境音桥接\n上一镜结尾的环境音元素，应在下一镜中自然延续或有明确的过渡说明。参照 L-cut 原理：声音可以比画面更早进入或更晚退出。\n\n### 原则 3.2 — 静默跳变须有理由\n如果 Shot N 结尾是"风声呼啸"，Shot N+1 开头是"一片寂静"——必须在 changes 中说明叙事理由（如"角色进入室内并关上门"）。无理由的静默跳变 → 修正。\n\n### 原则 3.3 — 环境音字段格式保持\noverall_soundscape 保持时间段标签格式 "[0.0s-3.0s] 环境描述"。标签后是叙事描述，不是音效关键词列表。\n正确: "[0.0s-3.0s] 营区内低沉的士兵交谈声与远处旗杆绳索的吱嘎声"\n错误: "0-3s: 士兵交谈, 旗杆吱嘎声"\n\n### 自检\n对每对相邻镜头，检查环境音过渡：\n- PASS: 上一镜结尾的音频元素在下一镜开头有延续/过渡\n- FAIL: 无理由的音频元素完全消失 → 添加过渡描述\n\n## 域4: 全局节奏 (PACING)\n\n### 原则 4.1 — 节奏曲线\n全剧集应有情绪起伏曲线。每个镜头标注在曲线中的功能角色：hook / buildup / tension / climax / release / transition / resolution\n\n### 原则 4.2 — 连续高潮控制\n连续 climax 不超过 3 个。超过时插入 release 或 transition 缓冲。\n\n## 保留规则 (PRESERVATION — 绝对不能修改)\n- subject_definitions 中的角色外观描述\n- retention_analysis\n- 对白文本（<d>...</d> 中的全部内容）\n- 运镜指令（Camera: xxx 及 H3 运镜术语）\n- 动作描述的主体逻辑和叙事结构\n- <Subject N> / <Picture N> 引用格式\n\n以下内容可以修改（且往往需要修改）：\n- non_diegetic_music（核心优化目标）\n- overall_soundscape（环境音过渡优化）\n- detailed_description 中的光照/色调描述词（视觉连贯性优化）\n- summary（如有宏观视角调整需要）\n\n\n\n# OUTPUT FORMAT (strict JSON)\n\nOutput ONLY the changed fields per shot — NOT the full video_prompt.\nnon_diegetic_music is REQUIRED for every shot. overall_soundscape is null if unchanged.\n\nJSON Schema:\n{\n  "domain_analysis": { ... },\n  "self_check": { ... },\n  "shots": [\n    {\n      "sequence": 1,\n      "role": "hook",\n      "non_diegetic_music": "完整配乐描述（必填）",\n      "overall_soundscape": null,\n      "changes": ["改动+理由"]\n    }\n  ]\n}\n\n# FEW-SHOT EXAMPLES\n\n## Scenario 1: L-cut continuity (most common)\nInput: Shot1 "6.0s渐弱至无声", Shot2 "极轻(pp)开始" — music restart at boundary\nOutput: Shot1 "持续至镜头结束", Shot2 "接续上一镜...,持續至镜头结束"\nReason: Same scene, dissolve — no narrative reason to cut\n\n## Scenario 2: Narrative silence (legitimate)\nInput: Shot5 "骤然停止，令人窒息的寂静"\nOutput: PRESERVE the silence\nReason: Calm before storm — legitimate narrative choice\n\n## Scenario 3: Audio bridge\nInput: Shot3 soundscape "营区外风声", Shot4 "帐内一片寂静"\nOutput: Shot4 "开门后风声涌入再减弱，转入帐内寂静"\nReason: Add transition to bridge the audio gap（暴风雨前的宁静）。\n\nShot 1 (role: buildup)\nnon_diegetic_music: 古筝极轻(pp)开始，3.0s渐强至中弱(mp)，持续至视频结束，不断开。\nchanges: ["配乐改为L-cut延续——同场景dissolve转场，无叙事理由切断", "音乐弧线: 古筝主题开场，为Shot2箫声叠入做铺垫"]\n\nShot 2 (role: buildup)\nnon_diegetic_music: 接续上一镜古筝，中弱(mp)保持，2.0s箫声叠入，渐强至中强(mf)，持续至视频结束。\nchanges: ["箫声在2.0s叠入——剧情需要（新角色出场），非镜头切换所致"]\n\n## 优化前（有叙事理由的沉默 — 合法，不修改）\nShot 5 (场景: 城墙，转入cut，转出cut)\nnon_diegetic_music: 打击乐持续渐强至中强(mf)，6.0s处骤然停止，留下一段令人窒息的寂静。\n\nShot 6 (场景: 城墙，转入cut，转出cut)\nnon_diegetic_music: 接续上一镜的寂静，2.0s后低音鼓极轻(pp)进入，渐强至中强(mf)。\nchanges: ["Shot5的沉默是合法叙事选择（暴风雨前的宁静——敌军即将破城），保留", "Shot6从沉默中渐入鼓声——制造对比效果"]`;

const VIDEO_PROMPT_OPTIMIZE_USER_TEMPLATE = `=== 剧集上下文 ===
标题: {title}
视觉风格: {visualStyle}
时代美学: {eraAesthetic}
情绪方向: {moodDirection}
...
=== 输出要求 ===
以上述 METHODOLOGY 和 CONSTITUTION 为准，输出你的完整优化结果。
每个 shot 的 video_prompt 必须是完整的 6 段格式。
保持 subject_definitions / retention_analysis / 对白 / 运镜 不变。
优化 non_diegetic_music / overall_soundscape / 光照描述 / summary。

严格输出 JSON（无 markdown 包裹，无注释），格式见下方 JSON Schema。`;

const videoPromptOptimizeDef: PromptDefinition = {
  key: "video_prompt_optimize",
  nameKey: "promptTemplates.prompts.videoPromptOptimize",
  descriptionKey: "promptTemplates.prompts.videoPromptOptimizeDesc",
  category: "video",
  slots: [
    slot("system", VIDEO_PROMPT_OPTIMIZE_SYSTEM, true),
    slot("user_template", VIDEO_PROMPT_OPTIMIZE_USER_TEMPLATE, true),
  ],
  buildFullPrompt() {
    return "";
  },
