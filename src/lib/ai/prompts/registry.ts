...
[shot] 景别 + 角度 + 焦段
  如: "全景，平视广角，35mm"
[era] 时代背景
  优先级: 严格引用用户消息顶部的「时代锚定：<文字>」段 <era_text>，直接复制不改编
  · 若用户消息有「时代锚定」→ 从中提取朝代/年代描述，精简为 8-20 字一行
  · 若无 → 从分镜列表中推断，不得泛化为"古代""近代"
  格式示例: "明初洪武年间，木构铁甲军旅" / "2020年代上海，霓虹雨夜" / "上古修真纪元"
  ⚠️ 禁止: 不要写成"古代中国"这类无时代信息的泛化词
...
[atmosphere] 大气粒子（雾/尘/烟/雨）+ 地面状态 + 天气 + 季节特征
  如: "干燥黄尘薄雾，地面散落箭矢碎木"
[style] 艺术风格 + 画幅比
  优先级: 严格引用用户消息顶部的「视觉风格基调：<文字>」节，直接复制核心描述不改编
  · 若用户消息有「视觉风格基调」→ 提取风格名称+材质描述+画幅比
  · 若无 → 从上下文推断，但必须包含画幅比
